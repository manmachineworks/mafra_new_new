            ...
            <!-- <div class="aiz-topbar-item mr-3">
                <div class="d-flex align-items-center">
                    <button id="refresh_shiprocket_token" class="btn btn-topbar has-transition btn-icon btn-circle btn-light p-0 hov-bg-primary hov-svg-white d-flex align-items-center justify-content-center"
                        ...
                        <span class="token-refresh-icon d-flex align-items-center justify-content-center">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
                                <path d="M8 0a8 8 0 0 0-8 8h1.6A6.4 6.4 0 0 1 14.4 8V6.4L16 8l-1.6 1.6V8A8 8 0 0 0 8 0Zm0 16a8 8 0 0 0 8-8h-1.6A6.4 6.4 0 0 1 1.6 8v1.6L0 8l1.6-1.6V8A8 8 0 0 0 8 16Z" fill="#717580"/>
                            </svg>
                        </span>
...
<script>
    (function() {
        var btn = document.getElementById('refresh_shiprocket_token');
        if (!btn) return;
        ...
        function doRefresh(showNotify) {
            ...
            $.post(refreshEndpoint, { _token: csrf })
                ...
                .fail(function(xhr) {
                    var message = (xhr.responseJSON && xhr.responseJSON.message) ? xhr.responseJSON.message : "{{ translate('Failed to refresh Shiprocket token') }}";
                    if (showNotify !== false) {
                        AIZ.plugins.notify('danger', message);
                    }
                })
        ...
        // Auto refresh once per 45 minutes in admin area to keep token warm.
        var last = parseInt(localStorage.getItem(lastRefreshedKey) || '0', 10);
