                ...
                <table class="table aiz-table mb-0">
                    <thead>
                        <tr>
                            ...
                            <th data-breakpoints="md">{{ translate('Shipment') }}</th>
                    ...
                    <tbody>
                        @foreach ($orders as $key => $order)
                            <tr>
                                ...
                                <td>
                                    @php
                                        $shipStatus = $order->trackingStatus();
                                        $badgeClass = match ($shipStatus) {
                                            'delivered' => 'badge-success',
                                            'out_for_delivery', 'shipped', 'in_transit' => 'badge-info',
                                            'cancelled' => 'badge-danger',
                                            default => 'badge-secondary'
                                        };
                                    @endphp
                                    <span class="badge badge-inline {{ $badgeClass }} mb-1">
                                        {{ $order->trackingStatusLabel() }}
                                    </span>
                                    @if ($order->shiprocket_awb)
                                        <button type="button" class="btn btn-xs btn-outline-primary ml-1 refresh-tracking" data-id="{{ $order->id }}">
                                            {{ translate('Refresh') }}
                                        </button>
                                    @endif
                                </td>
...
@section('script')
    <script type="text/javascript">
        ...
        $('.refresh-tracking').on('click', function () {
            ...
            var url = '{{ route('order.track', ':id') }}'.replace(':id', orderId);
            $.get(url, {}, function (resp) {
                btn.closest('td').find('.badge').text(resp.status_label || resp.status);
                AIZ.plugins.notify('success', resp.message || "{{ translate('Tracking updated') }}");
            }).fail(function () {
                AIZ.plugins.notify('danger', "{{ translate('Failed to refresh tracking') }}");
            }).always(function () {
