@php
    ...
    // Product images (show a single image by default, swap to the 2nd image on hover if available)
    $photos = collect(explode(',', (string) $product->photos))->filter()->values();
    $primary_image = get_image($product->thumbnail);
    $secondary_image = null;

    if ($photos->count() > 1) {
        $secondary_image = uploaded_asset($photos[1]);
    } else {
        $stock_image = optional($product->stocks)->pluck('image')->filter()->first();
        if (!empty($stock_image)) {
            $secondary_image = uploaded_asset($stock_image);
        }
    }
...
<div class="aiz-card-box h-auto bg-white py-3 hov-scale-img product-card @if($secondary_image) has-hover-img @endif">
    <div class="position-relative h-140px h-md-200px img-fit overflow-hidden">

        <!-- Image -->
        <a href="{{ $product_url }}" class="d-block h-100 position-relative image-hover-effect">
            <img
                class="lazyload mx-auto img-fit has-transition product-card-img product-card-img--main"
                src="{{ $primary_image }}"
            ...
            @if($secondary_image)
                <img
                    class="lazyload mx-auto img-fit has-transition product-card-img product-card-img--hover"
                    src="{{ $secondary_image }}"
                    alt="{{ $product->getTranslation('name') }}"
                    title="{{ $product->getTranslation('name') }}"
                    onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.jpg') }}';">
